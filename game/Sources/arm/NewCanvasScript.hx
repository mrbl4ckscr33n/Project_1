	...
	static public function drawElement(ui: Zui, canvas: Tcanvas, element: Telement, px = 0.0, py = 0.0)
	{
		...
		switch (element.type)
		{
			...
			case Container:
				...
				var minX = new Array <kha.FastFloat> ();
				var maxX = new Array <kha.FastFloat> ();
				var minY = new Array <kha.FastFloat> ();
				var maxY = new Array <kha.FastFloat> ();

				for(h in 0...canvas.elements.length)
				{
					if((canvas.elements[h].type == Container) && (canvas.elements[h].id != element.id))
					{
						minX.push(canvas.elements[h].x - canvas.elements[h].width);
						maxX.push((canvas.elements[h].alignmentX) * canvas.elements[h].width + canvas.elements[h].x);
						minY.push(canvas.elements[h].y - canvas.elements[h].height);
						maxY.push((canvas.elements[h].alignmentY) * canvas.elements[h].height + canvas.elements[h].y);
					}
				}
				...
				for(h in 0...canvas.elements.length)
				{
					...
					{
						for(k in 0...minX.length)
						{
							if(canvas.elements[h].x >= minX[k] && canvas.elements[h].x <= maxX[k] && canvas.elements[h].y >= minY[k] && canvas.elements[h].y <= maxY[k]) 
							{
								//trace(element.name, ": ", canvas.elements[h].id, " is hovering over another container");
								broke = true;
								break;
							}
						}

						//trace(element.name, broke, " ", canvas.elements[h].asset);
						if(broke) continue;
						//else trace(element.name, ": ", canvas.elements[h].id, " is hovering over this container");
						...
						{
							while(currentY < element.height * element.alignmentY + element.y)
							{
								currentX = element.x;
								while(currentX < element.width * element.alignmentX + element.x)
								{
									used = false;
						...
						else if((canvas.elements[h].lastX != canvas.elements[h].x) || (canvas.elements[h].lastY != canvas.elements[h].y))
						{
							var totalSizeX: Int = (element.width * Std.int(element.alignmentX) - element.width + Std.int(element.x));
							var totalSizeY: Int = (element.height * Std.int(element.alignmentY) - element.height + Std.int(element.y));

							canvas.elements[h].x = Math.round(canvas.elements[h].x/element.width) * element.width;
							canvas.elements[h].y = Math.round(canvas.elements[h].y/element.height) * element.height;
				...
				if (image != null && !fontAsset)
					...
					ui.imageScrollAlign = false;
					var tint = element.color != null ? element.color : 0xffffffff;
					if (ui.image(image, tint, scaled(element.height * element.alignmentY)) == zui.Zui.State.Released)
					{
						var e = element.event;
						if (e != null && e != "") events.push(e);
					}
...
typedef Telement = {
	...
	@:optional var alignmentX: Null<Float>;
	@:optional var alignmentY: Null<Float>;
}
